...
window.closeBulkImportModal = function () {
    ...
    setTimeout(() => {
        modal.classList.add('hidden');
        modal.classList.remove('flex');
        // If results were shown, reload to update the UI
        if (!document.getElementById('pedagogBulkResults').classList.contains('hidden')) {
...
window.handleCreateStudent = async function (event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            closeStudentModal();
            setTimeout(() => window.location.reload(), 1500); // Wait for toast
        } else {
